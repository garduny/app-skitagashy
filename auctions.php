...
$auctions = getQuery(" SELECT a.id,a.end_time,a.current_bid,a.start_price,a.status,p.title,p.images,p.slug,p.description,u.username as high_bidder FROM auctions a JOIN products p ON a.product_id=p.id LEFT JOIN users u ON a.highest_bidder_id=u.id WHERE a.status='active' AND a.end_time>NOW() ORDER BY a.end_time ASC ");
?>
