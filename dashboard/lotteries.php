...
if (post('new_round')) {
    $exists = findQuery(" SELECT id FROM lottery_rounds WHERE status='open' ORDER BY id DESC LIMIT 1 ");
    if (!$exists) {
        $last = findQuery(" SELECT MAX(round_number) r FROM lottery_rounds ");
        $next = ((int)($last['r'] ?? 0)) + 1;
        $draw = secure(request('draw_time', 'post'));
        if ($draw) {
        ...
        redirect('lotteries.php?msg=active_exists');
    }
}
$status = secure(request('status', 'get'));
$page = max(1, (int)(request('page', 'get') ?: 1));
$limit = 10;
$offset = ($page - 1) * $limit;
$where = " WHERE 1=1 ";
if ($status === 'open' || $status === 'closed') $where .= " AND status='$status' ";
...
<main class="ml-0 lg:ml-64 pt-20 p-6 min-h-screen transition-all duration-300">
    ...
    <?php if (request('msg', 'get') === 'active_exists'): ?>
        <div class="mb-6 p-4 rounded-xl bg-red-500/10 text-red-500 font-bold">An open round already exists.</div>
    <?php endif; ?>
    <?php if (request('msg', 'get') === 'created'): ?>
        <div class="mb-6 p-4 rounded-xl bg-green-500/10 text-green-500 font-bold">New round created successfully.</div>
    <?php endif; ?>
    <div class="bg-white dark:bg-dark-800 rounded-2xl border border-gray-200 dark:border-white/5 shadow-sm overflow-hidden mb-6">
        ...
        <div class="p-4 border-t border-gray-200 dark:border-white/5 flex justify-center gap-2">
            <?php if ($pages > 1): for ($i = 1; $i <= $pages; $i++): ?>
                    <a href="?page=<?= $i ?><?= $status ? '&status=' . $status : '' ?>" class="px-3 py-1 rounded-lg text-sm font-bold <?= $i === $page ? 'bg-primary-500 text-white' : 'bg-gray-100 dark:bg-white/5 text-gray-500 hover:bg-gray-200 dark:hover:bg-white/10' ?>"><?= $i ?></a>
            <?php endfor;
            endif; ?>
        </div>
    </div>
...
<div id="addModal" class="fixed inset-0 z-[60] hidden">
    ...
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white dark:bg-dark-800 rounded-2xl shadow-2xl p-6">
        ...
        <form method="POST">
            <div class="space-y-4">
                <div>
                    <label class="block text-xs font-bold text-gray-500 uppercase mb-1">Draw Date</label>
                    <input type="datetime-local" name="draw_time" required class="w-full bg-gray-50 dark:bg-dark-900 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-gray-900 dark:text-white outline-none">
                </div>
