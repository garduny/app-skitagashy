...
$stats = findQuery(" SELECT COUNT(*) as total_items, SUM(stock) as total_stock FROM products WHERE seller_id=$id ");
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white dark:bg-dark-800 rounded-2xl border border-gray-200 dark:border-white/5 p-6 shadow-sm text-center">
                <div class="w-24 h-24 mx-auto rounded-full bg-gradient-to-tr from-primary-500 to-blue-500 p-1 mb-4">
                    <div class="w-full h-full bg-white dark:bg-dark-800 rounded-full flex items-center justify-center text-2xl font-bold uppercase"><?= substr($seller['store_name'], 0, 1) ?></div>
                </div>
                ...
                <div class="grid grid-cols-2 gap-4 border-t border-gray-100 dark:border-white/5 pt-4">
                    <div class="text-center">
                        ...
                        <div class="text-lg font-bold text-gray-900 dark:text-white"><?= number_format($seller['total_sales']) ?></div>
                    </div>
                    <div class="text-center">
            ...
            <div class="bg-white dark:bg-dark-800 rounded-2xl border border-gray-200 dark:border-white/5 p-6 shadow-sm">
                ...
                <div class="space-y-3 text-sm">
                    ...
                    <div class="flex justify-between"><span class="text-gray-500">Wallet</span> <span class="text-primary-500 font-mono text-xs"><?= substr($seller['wallet_address'], 0, 6) ?>...</span></div>
                </div>
        ...
        <div class="lg:col-span-2">
            <div class="bg-white dark:bg-dark-800 rounded-2xl border border-gray-200 dark:border-white/5 p-6 shadow-sm overflow-hidden">
                <div class="flex justify-between items-center mb-6">
                    <h3 class="font-bold text-gray-900 dark:text-white">Products (<?= $stats['total_items'] ?>)</h3>
                </div>
                        ...
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            <?php foreach ($products as $p): $img = json_decode($p['images'])[0] ?? ''; ?>
                                <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                                    <td class="py-3 pl-2">
                                        <div class="flex items-center gap-3"><img src="<?= $img ?>" class="w-8 h-8 rounded bg-gray-100 dark:bg-white/5 object-cover"><span class="font-bold text-gray-900 dark:text-white truncate max-w-[200px]"><?= $p['title'] ?></span></div>
                                    </td>
                                    <td class="py-3 font-mono"><?= number_format($p['price_gashy']) ?></td>
                                    ...
                                    <td class="py-3 pr-2 text-right"><a href="productdetail.php?id=<?= $p['id'] ?>" class="text-gray-400 hover:text-primary-500"><i class="fa-solid fa-pen"></i></a></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
