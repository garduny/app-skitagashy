...
$today = date('Y-m-d');
$stats = [
    'users' => countQuery(" SELECT COUNT(*) FROM accounts"),
    ...
    'orders_pending' => countQuery(" SELECT COUNT(*) FROM orders WHERE status='pending'"),
    'burned' => findQuery(" SELECT SUM(amount) as t FROM burn_log")['t'] ?? 0,
    'sellers' => countQuery(" SELECT COUNT(*) FROM sellers WHERE is_approved=1")
];
...
$top_products = getQuery(" SELECT p.title,SUM(oi.quantity) as sold,p.images FROM order_items oi JOIN products p ON oi.product_id=p.id GROUP BY oi.product_id ORDER BY sold DESC LIMIT 5");
$top_sellers = getQuery(" SELECT store_name,total_sales FROM sellers ORDER BY total_sales DESC LIMIT 5");
...
<main class="ml-0 lg:ml-64 pt-20 p-6 min-h-screen transition-all duration-300">
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="bg-white dark:bg-dark-800 p-6 rounded-2xl border border-gray-200 dark:border-white/5 shadow-sm relative overflow-hidden">
            <div class="absolute top-0 right-0 p-4 opacity-10"><i class="fa-solid fa-wallet text-4xl text-primary-500"></i></div>
            ...
            <div class="mt-2 text-xs text-green-500 font-bold"><i class="fa-solid fa-arrow-up"></i> +<?= number_format($stats['sales_today']) ?> Today</div>
        </div>
        <div class="bg-white dark:bg-dark-800 p-6 rounded-2xl border border-gray-200 dark:border-white/5 shadow-sm relative overflow-hidden">
            <div class="absolute top-0 right-0 p-4 opacity-10"><i class="fa-solid fa-fire text-4xl text-red-500"></i></div>
            ...
            <div class="mt-2 text-xs text-red-500 font-bold">Deflationary</div>
        </div>
        <div class="bg-white dark:bg-dark-800 p-6 rounded-2xl border border-gray-200 dark:border-white/5 shadow-sm relative overflow-hidden">
            <div class="absolute top-0 right-0 p-4 opacity-10"><i class="fa-solid fa-users text-4xl text-blue-500"></i></div>
            <div class="text-gray-500 text-xs font-bold uppercase mb-2">Total Users</div>
            <div class="text-3xl font-black text-gray-900 dark:text-white"><?= number_format($stats['users']) ?></div>
            <div class="mt-2 text-xs text-gray-400"><?= number_format($stats['sellers']) ?> Sellers</div>
        </div>
        <div class="bg-white dark:bg-dark-800 p-6 rounded-2xl border border-gray-200 dark:border-white/5 shadow-sm relative overflow-hidden">
            <div class="absolute top-0 right-0 p-4 opacity-10"><i class="fa-solid fa-clock text-4xl text-yellow-500"></i></div>
            <div class="text-gray-500 text-xs font-bold uppercase mb-2">Pending Orders</div>
            <div class="text-3xl font-black text-gray-900 dark:text-white"><?= number_format($stats['orders_pending']) ?></div>
            <div class="mt-2 text-xs text-yellow-500 font-bold">Action Required</div>
        </div>
    </div>
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <div class="lg:col-span-2 bg-white dark:bg-dark-800 rounded-2xl border border-gray-200 dark:border-white/5 shadow-sm p-6">
            <div class="flex justify-between items-center mb-6">
                <h3 class="font-bold text-gray-900 dark:text-white">Revenue (Last 7 Days)</h3><span class="text-xs font-bold text-primary-500 bg-primary-500/10 px-2 py-1 rounded">Live Data</span>
            </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-dark-800 rounded-2xl border border-gray-200 dark:border-white/5 shadow-sm p-6">
            <h3 class="font-bold text-gray-900 dark:text-white mb-6">Recent Orders</h3>
            <div class="space-y-4">
            ...
            <a href="orders.php" class="block mt-4 text-center text-xs font-bold text-primary-500 hover:text-primary-400">View All Orders</a>
        </div>
        <div class="bg-white dark:bg-dark-800 rounded-2xl border border-gray-200 dark:border-white/5 shadow-sm p-6">
            <h3 class="font-bold text-gray-900 dark:text-white mb-6">Top Selling Products</h3>
            <div class="space-y-4">
                <?php foreach ($top_products as $tp): $img = json_decode($tp['images'])[0] ?? ''; ?>
                    <div class="flex items-center gap-3 pb-4 border-b border-gray-100 dark:border-white/5 last:border-0 last:pb-0">
                        ...
                        <div class="flex-1 min-w-0">
                            ...
                            <div class="text-xs text-gray-500"><?= $tp['sold'] ?> Sold</div>
                        </div>
                        <div class="text-primary-500 font-bold text-sm">#1</div>
                    </div>
                <?php endforeach; ?>
            </div>
        </div>
            ...
            <div class="space-y-4">
                <?php foreach ($top_sellers as $i => $ts): ?>
                    <div class="flex items-center justify-between pb-4 border-b border-gray-100 dark:border-white/5 last:border-0 last:pb-0">
                        ...
                        <div class="text-xs font-bold text-gray-500"><?= $ts['total_sales'] ?> Sales</div>
                    </div>
                <?php endforeach; ?>
            </div>
        </div>
