...
if (post('draw_winner') && $r['status'] === 'open') {
    ...
    foreach ($entries as $e) {
        for ($i = 0; $i < (int)$e['ticket_count']; $i++) $pool[] = (int)$e['account_id'];
    }

    execute(" UPDATE lottery_entries SET is_winner=0 WHERE round_id=$id ");

    if (empty($pool)) {
        execute(" UPDATE lottery_rounds SET status='closed' WHERE id=$id ");
    } else {

        $total = (float)$r['prize_pool'];
        $tiers = [0.5, 0.3, 0.2];
        $winners = [];
        $used = [];

        foreach ($tiers as $rank => $p) {

            $available = array_values(array_filter($pool, function ($x) use ($used) {
                return !in_array($x, $used);
            }));
            if (empty($available)) break;

            $key = array_rand($available);
            $uid = (int)$available[$key];
            $used[] = $uid;

            $amount = $total * $p;
            $rankNum = $rank + 1;

            $winners[] = ['rank' => $rankNum, 'user' => $uid, 'amount' => $amount];

            execute(" UPDATE lottery_entries SET is_winner=1 WHERE round_id=$id AND account_id=$uid ");

            execute(" INSERT INTO transactions (account_id,type,amount,reference_id,status,created_at) VALUES ($uid,'reward',$amount,$id,'confirmed',NOW()) ");
        }

        $win_json = json_encode($winners);
                ...
                <table class="w-full text-left text-sm">
                    ...
                    <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                        ...
                        ?>
                            ...
                            <tr class="<?= $e['is_winner'] ? 'bg-green-500/10' : '' ?>">
