    ...
    if (!$email || !$pass) {
        ...
        if ($u && password_verify($pass, $u['password'])) {
            if ($u['is_active'] == 1) {
                ...
                execute(" UPDATE users SET updated_at=NOW() WHERE id={$u['id']} ");
...
<body class="bg-gray-50 dark:bg-dark-900 text-gray-900 dark:text-white flex items-center justify-center min-h-screen transition-colors">
    <div class="w-full max-w-md p-8 space-y-6 bg-white dark:bg-dark-800 rounded-2xl shadow-xl border border-gray-200 dark:border-white/5">
        ...
        <form method="POST" action="" class="space-y-5">
            ...
            <button type="submit" name="submit_login" value="1" class="w-full py-3.5 bg-primary-600 hover:bg-primary-500 text-white font-bold rounded-xl shadow-lg shadow-primary-500/25 transition-all transform hover:-translate-y-0.5">Sign In</button>
        </form>
        <div class="text-center">
