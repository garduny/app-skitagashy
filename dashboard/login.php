...
if (post('submit_otp')) {
    ...
    if ($u && $u['otp_code'] === $code && strtotime($u['otp_expires']) > time()) {
    ...
    <div class="w-full max-w-md p-8 space-y-6 bg-white dark:bg-dark-800 rounded-2xl shadow-xl border border-gray-200 dark:border-white/5">
        ...
        <?php if (isset($step) && $step == 2): ?>
            <form method="POST" action="" class="space-y-5">
                ...
                <button type="submit" name="submit_otp" value="1" class="w-full py-3.5 bg-primary-600 hover:bg-primary-500 text-white font-bold rounded-xl transition-all">Verify & Login</button>
            </form>
        <?php else: ?>
            <form method="POST" action="" class="space-y-5">
                <div><label class="block text-xs font-bold text-gray-500 uppercase mb-2">Email Address</label><input type="email" name="email" required class="w-full px-4 py-3 bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-xl focus:border-primary-500 outline-none text-gray-900 dark:text-white"></div>
                <div>
                    <div class="flex justify-between items-center mb-2"><label class="block text-xs font-bold text-gray-500 uppercase">Password</label><a href="forgetpassword.php" class="text-xs text-primary-500 hover:underline">Forgot?</a></div><input type="password" name="password" required class="w-full px-4 py-3 bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-xl focus:border-primary-500 outline-none text-gray-900 dark:text-white">
                </div>
                <button type="submit" name="submit_login" value="1" class="w-full py-3.5 bg-primary-600 hover:bg-primary-500 text-white font-bold rounded-xl transition-all">Continue</button>
            </form>
        <?php endif; ?>
        <div class="text-center">
