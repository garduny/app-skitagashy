...
$total = countQuery(" SELECT COUNT(*) FROM sellers s JOIN accounts a ON s.account_id=a.id $where ");
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    <?php foreach ($sellers as $s): ?>
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                            ...
                            <td class="px-6 py-4 font-bold"><?= $s['total_sales'] ?></td>
                            ...
                            <td class="px-6 py-4 text-right">
                                <?php if (!$s['is_approved']): ?><a href="?approve=<?= $s['account_id'] ?>" class="px-3 py-1 bg-green-600 hover:bg-green-500 text-white rounded-lg text-xs font-bold mr-2">Approve</a><?php else: ?><a href="?reject=<?= $s['account_id'] ?>" class="px-3 py-1 bg-yellow-600 hover:bg-yellow-500 text-white rounded-lg text-xs font-bold mr-2">Suspend</a><?php endif; ?>
                                <a href="?delete=<?= $s['account_id'] ?>" onclick="return confirm('Delete store?')" class="p-2 text-gray-400 hover:text-red-500"><i class="fa-solid fa-trash"></i></a>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                </tbody>
