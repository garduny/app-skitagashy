...
if ($status === 'approved') $where .= " AND s.is_approved=1 ";
if ($status === 'pending') $where .= " AND s.is_approved=0 ";
if ($status === 'banned') $where .= " AND a.is_banned=1 ";
...
function pageUrlSeller($page, $search, $status)
{
    return '?' . http_build_query(['page' => $page, 'search' => $search, 'status' => $status]);
}
$msg = request('msg', 'get');
...
<main class="ml-0 lg:ml-64 pt-20 p-6 min-h-screen transition-all duration-300">
    <div class="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
        ...
        <div class="flex gap-2 flex-wrap">
            <a href="?status=approved" class="px-4 py-2 rounded-xl text-sm font-bold bg-green-500/10 text-green-500">Approved</a>
            <a href="?status=pending" class="px-4 py-2 rounded-xl text-sm font-bold bg-yellow-500/10 text-yellow-500">Pending</a>
            <a href="?status=banned" class="px-4 py-2 rounded-xl text-sm font-bold bg-red-500/10 text-red-500">Banned</a>
            <a href="sellers.php" class="px-4 py-2 rounded-xl text-sm font-bold bg-gray-100 dark:bg-white/5 text-gray-500">Reset</a>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-dark-800 rounded-2xl border border-gray-200 dark:border-white/5 shadow-sm overflow-hidden">
        <div class="p-4 border-b border-gray-200 dark:border-white/5">
            <form class="grid md:grid-cols-[1fr_220px_50px] gap-3">
                    ...
                    ?>
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5">
                            <td class="px-6 py-4 min-w-[240px]">
                                <div class="font-bold text-gray-900 dark:text-white"><?= htmlspecialchars($s['store_name']) ?></div>
                                <div class="text-xs text-primary-500">@<?= htmlspecialchars($s['store_slug']) ?></div>
                            </td>
                            <td class="px-6 py-4 min-w-[250px]">
                                <div class="font-bold text-gray-900 dark:text-white"><?= htmlspecialchars($s['accountname']) ?></div>
                                <div class="text-xs text-gray-500"><?= htmlspecialchars($s['email']) ?></div>
                                <div class="text-xs text-gray-400 font-mono"><?= htmlspecialchars(substr($s['wallet_address'], 0, 8)) ?>...</div>
                            </td>
                            <td class="px-6 py-4">
                                <div class="font-bold text-primary-500"><?= number_format($sales, 2) ?> GASHY</div>
                                <div class="text-xs text-gray-500">Tier: <?= strtoupper($s['tier']) ?></div>
                            </td>
                            <td class="px-6 py-4 font-bold"><?= number_format((float)$s['commission_rate'], 2) ?>%</td>
                            <td class="px-6 py-4 text-sm text-gray-500"><?= !empty($s['account_created']) ? date('Y-m-d', strtotime($s['account_created'])) : '-' ?></td>
                            <td class="px-6 py-4">
                            ...
                            <td class="px-6 py-4 text-right whitespace-nowrap">
                                <a href="../seller.php?slug=<?= urlencode($s['store_slug']) ?>" target="_blank" class="p-2 text-gray-400 hover:text-blue-500"><i class="fa-solid fa-eye"></i></a>
                                <a href="sellerdetail.php?id=<?= $s['account_id'] ?>" class="p-2 text-gray-400 hover:text-purple-500"><i class="fa-solid fa-boxes-stacked"></i></a>
                                <button type="button" onclick="navigator.clipboard.writeText('<?= htmlspecialchars($s['wallet_address'], ENT_QUOTES) ?>')" class="p-2 text-gray-400 hover:text-emerald-500"><i class="fa-solid fa-copy"></i></button>
                                ...
                                <?php endif; ?>
                                <button onclick="confirmDelete(<?= $s['account_id'] ?>)" class="p-2 text-gray-400 hover:text-red-500 ml-1"><i class="fa-solid fa-trash"></i></button>
                            </td>
                        </tr>
                    <?php endforeach; ?>
                    <?php if (!$sellers): ?>
                        <tr>
                            <td colspan="7" class="px-6 py-10 text-center text-gray-400">No sellers found</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
        ...
        <?php if ($pages > 1): ?>
            <div class="p-4 border-t border-gray-200 dark:border-white/5 flex flex-wrap justify-center gap-2">
                <?php for ($i = 1; $i <= $pages; $i++): ?>
                    <a href="<?= pageUrlSeller($i, $search, $status) ?>" class="px-3 py-1 rounded-lg text-sm font-bold <?= $i == $page ? 'bg-primary-500 text-white' : 'bg-gray-100 dark:bg-white/5 text-gray-500' ?>"><?= $i ?></a>
                <?php endfor; ?>
            </div>
        <?php endif; ?>
    </div>
