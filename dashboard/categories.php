...
if (get('delete')) {
    ...
    if ($check) {
        redirect('categories.php?msg=error_has_products');
    }
...
if (post('add_cat')) {
    ...
    if ($check) {
        redirect('categories.php?msg=exists');
    } else {
        $icon = secure($_POST['icon']);
...
$cats = getQuery(" SELECT c.*,(SELECT COUNT(*) FROM products WHERE category_id=c.id) as prod_count FROM categories c ORDER BY c.id ASC ");
                ...
                <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                    <?php foreach ($cats as $c): ?>
                        <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
                            <td class="px-6 py-4">
                                <div class="w-10 h-10 rounded-lg bg-gray-100 dark:bg-white/10 flex items-center justify-center text-gray-600 dark:text-gray-300"><i class="<?= $c['icon'] ?> text-lg"></i></div>
                            </td>
    ...
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white dark:bg-dark-800 rounded-2xl shadow-2xl p-6">
        ...
        <form method="POST">
            <div class="space-y-4">
                ...
                <div><label class="block text-xs font-bold text-gray-500 uppercase mb-1">Icon (FontAwesome)</label><input type="text" name="icon" placeholder="fa-solid fa-tag" required class="w-full bg-gray-50 dark:bg-dark-900 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-gray-900 dark:text-white outline-none"></div><button type="submit" name="add_cat" value="1" class="w-full py-3 bg-primary-600 hover:bg-primary-500 text-white font-bold rounded-xl">Create</button>
            </div>
        </form>
    </div>
</div>
    ...
    <div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md bg-white dark:bg-dark-800 rounded-2xl shadow-2xl p-6">
        ...
        <form method="POST"><input type="hidden" name="id" id="edit_id">
            <div class="space-y-4">
                ...
                <div><label class="block text-xs font-bold text-gray-500 uppercase mb-1">Icon (FontAwesome)</label><input type="text" name="icon" id="edit_icon" required class="w-full bg-gray-50 dark:bg-dark-900 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-gray-900 dark:text-white outline-none"></div>
                <div><label class="block text-xs font-bold text-gray-500 uppercase mb-1">Status</label><select name="is_active" id="edit_status" class="w-full bg-gray-50 dark:bg-dark-900 border border-gray-200 dark:border-white/10 rounded-xl px-4 py-2.5 text-gray-900 dark:text-white outline-none">
                    ...
                    </select></div><button type="submit" name="edit_cat" value="1" class="w-full py-3 bg-primary-600 hover:bg-primary-500 text-white font-bold rounded-xl">Save Changes</button>
            </div>
        </form>
    </div>
</div>
