...
if (!window.App) {
    window.App = {
        ...
        async fetchProfile() {
            try {
                const res = await this.post('api/user/profile.php', {});
                if (res.status) {
        ...
        renderUI() {
            ...
            if (this.state.user) {
                const w = this.state.user.wallet_address;
                ...
                if (userInfo) {
                    ...
                    balanceText.innerText = (this.state.user.tier_progress?.current || '0.00') + ' GASHY';
                }
            } else {
