...
async function loadUserData() {
    try {
        // Fetch User Info
        const res = await App.post('api/user/profile.php', {});
        if (res.status && res.data) {
            const u = res.data;

            // Populate DOM
            document.getElementById('profile-username').innerText = u.username || 'Trader';
            ...
            document.getElementById('referral-code').value = 'GASHY-REF-User' + u.id; // Or real ref code if API returns it

            // Tier Icon
            const tiers = { 'bronze': '🥉', 'silver': '🥈', 'gold': '🥇', 'platinum': '💎', 'diamond': '👑' };
            document.getElementById('user-tier-icon').innerText = tiers[u.tier] || '🥉';
        }
    } catch (e) {
...
async function saveProfile() {
    ...
    try {
        const res = await App.post('api/user/update.php', { username, email });
        if (res.status) {
            notyf.success(res.message);
            document.getElementById('profile-username').innerText = username || 'User';
        } else {
