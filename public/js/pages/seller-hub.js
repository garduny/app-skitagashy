...
function renderTable() {
    ...
    list.innerHTML = myProducts.map(p => {
        ...
        <tr class="hover:bg-gray-50 dark:hover:bg-white/5 transition-colors">
            <td class="px-6 py-4"><div class="flex items-center gap-3"><img src="${img}" class="w-10 h-10 rounded-lg object-cover bg-gray-100 dark:bg-white/5"><div class="font-bold text-gray-900 dark:text-white truncate max-w-[200px]">${p.title}</div></div></td>
            <td class="px-6 py-4 font-mono font-bold text-gray-900 dark:text-white">${parseFloat(p.price_gashy).toFixed(2)}</td>
            <td class="px-6 py-4">${p.stock}</td>
            <td class="px-6 py-4 text-xs uppercase">${p.cat_name || '-'}</td>
            <td class="px-6 py-4"><span class="px-2 py-1 rounded text-[10px] uppercase font-bold ${p.status === 'active' ? 'text-green-500 bg-green-500/10' : 'text-red-500 bg-red-500/10'}">${p.status}</span></td>
...
function editProduct(id) {
    ...
    document.getElementById('prod-images').value = JSON.parse(p.images).join('\n');
...
async function saveProduct() {
    ...
    } catch (e) { notyf.error('Save failed'); }
}
async function deleteProduct(id) {
    if (!confirm('Remove this product?')) return;
    try {
        const res = await App.post('api/seller/save.php', { action: 'delete', id: id });
        if (res.status) { notyf.success('Deleted'); loadHub(); }
