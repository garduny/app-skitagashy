...
function renderTable() {
    ...
    list.innerHTML = myProducts.map(p => {
        ...
        const gashy = gashyRate ? usd / gashyRate : 0
        return `<tr>
<td><div style="display:flex;align-items:center;gap:10px"><img src="${img}" class="prod-img"><span class="prod-title">${p.title}</span></div></td>
<td><div style="display:flex;flex-direction:column"><span class="mono" style="font-size:.8rem;font-weight:700">$${usd.toFixed(2)}</span><span class="mono" style="font-size:.7rem;color:var(--muted)">${gashy.toFixed(2)} G</span></div></td>
<td><span class="mono" style="font-size:.8rem">${p.stock}</span></td>
<td><span class="sh-badge ${statusClass}">${p.status}</span></td>
<td style="text-align:right"><div style="display:inline-flex;align-items:center;gap:2px">${getActionButtons(p)}</div></td>
</tr>`
    }).join('')
}
...
async function saveProduct() {
    ...
    if (j.status) { notyf.success(j.message); closeProductModal(); loadHub() } else notyf.error(j.message)
}

async function deleteProduct(id) {
    if (!confirm('Remove this product?')) return
    const res = await App.post('./api/seller/save.php', { action: 'delete', id })
    if (res.status) { notyf.success('Product deleted'); loadHub() } else notyf.error(res.message)
}
...
async function requestWithdraw() {
    ...
    if (res.status) { notyf.success(res.message); loadHub() } else notyf.error(res.message)
}
...
async function invAddOption() {
    ...
    if (!name) return
    const price = prompt('Option price USD')
    if (!price) return
    const res = await App.post('./api/seller/options.php', { product_id: _invProductId, action: 'add', name: name, price_usd: price })
    if (res.status) { notyf.success('Option added'); loadInvOptions() } else notyf.error(res.message)
}
...
async function loadInvOptions() {
    ...
    sel.innerHTML = ''
    if (!res.options || res.options.length === 0) { sel.innerHTML = `<option value="0">Default</option>`; return }
...
async function loadInvCodes() {
    ...
    if (!res.status) { notyf.error(res.message || 'Failed to load'); return }
...
<div style="display:flex;align-items:center;gap:10px">
<span class="sh-badge ${c.is_sold ? 'inactive' : 'active'}">${c.is_sold ? 'Sold' : 'Available'}</span>
${c.is_sold ? `` : `<button onclick="invDeleteCode(${c.id})" class="btn-icon del"></button>`}
</div>
</div>
`).join('')
}
...
async function invAddCodes() {
    ...
    if (!raw) { notyf.error('Enter at least one code'); return }
    ...
    if (res.status) { notyf.success(res.message || 'Codes imported'); document.getElementById('inv-codes-input').value = ''; loadHub(); loadInvCodes() } else notyf.error(res.message || 'Failed')
}
...
async function invDeleteCode(cid) {
    ...
    if (res.status) { notyf.success('Code removed'); loadInvCodes() } else notyf.error(res.message || 'Failed')
}
...
async function loadMysteryLoot() {
    ...
    if (!res.status) { notyf.error(res.message || 'Failed'); return }
...
<span class="sh-badge">${l.rarity}</span>
<div>
...
async function mysteryAddLoot() {
    ...
    if (!payload.probability) { notyf.error('Enter a probability'); return }
    const res = await App.post('./api/seller/mystery.php', payload)
    if (res.status) { notyf.success('Loot added'); loadMysteryLoot() } else notyf.error(res.message || 'Failed')
}
...
async function mysteryDeleteLoot(lid) {
    ...
    if (res.status) { notyf.success('Removed'); loadMysteryLoot() } else notyf.error(res.message || 'Failed')
}