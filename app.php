...
$trending = [];
try {
    $trending = getQuery(" SELECT p.id,p.title,p.slug,p.price_gashy,p.images,p.type,s.store_name,s.is_approved FROM products p JOIN sellers s ON p.seller_id=s.user_id WHERE p.status='active' AND p.stock>0 ORDER BY p.views DESC LIMIT 4");
} catch (Exception $e) {
}
?>
