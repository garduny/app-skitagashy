...
--
-- Host: 127.0.0.1
-- Generation Time: Jan 18, 2026 at 03:10 AM
...
(1, 7, '2026-01-17 15:23:19', '2026-01-19 15:23:19', 5000.000000000, NULL, 5500.000000000, 1, 'active');
...
INSERT INTO `burn_log` (`id`, `user_id`, `amount`, `purpose`, `tx_signature`, `usd_value_at_burn`, `created_at`) VALUES
...
INSERT INTO `lottery_entries` (`id`, `round_id`, `user_id`, `burn_tx`, `ticket_count`) VALUES
...
(1, 1, 100010.000000000, '2026-01-17 12:30:32', 'open', NULL);
...
INSERT INTO `orders` (`id`, `user_id`, `total_gashy`, `tx_signature`, `status`, `created_at`) VALUES
...
(2, 1, 1, 'Steam Wallet $20 Global', 'steam-20-global', 'Add funds to your Steam Wallet. Works globally. Instant delivery.', 480.000000000, 20.00, 50, 'gift_card', '[\"https://upload.wikimedia.org/wikipedia/commons/thumb/8/83/Steam_icon_logo.svg/2048px-Steam_icon_logo.svg.png\"]', NULL, 'active', 895, '2026-01-16 08:40:08'),
(3, 1, 2, 'PUBG Mobile 660 UC', 'pubg-660-uc', 'Redeemable code for PUBG Mobile. Get skins and upgrades instantly.', 240.000000000, 10.00, 200, 'digital', '[\"https://w7.pngwing.com/pngs/380/764/png-transparent-pubg-mobile-playerunknown-s-battlegrounds-logo-game-t-shirt-pubg-mobile-logo-game-text-logo-thumbnail.png\"]', NULL, 'active', 120, '2026-01-16 08:40:08'),
...
(5, 1, 4, 'CyberPunk Samurai #042', 'cyber-samurai-042', 'Rare NFT from the CyberPunk collection. Verified ownership on Solana.', 5000.000000000, 250.00, 1, 'nft', '[\"https://images.unsplash.com/photo-1620641788421-7a1c342ea42e?q=80&w=1974&auto=format&fit=crop\"]', NULL, 'active', 1202, '2026-01-16 08:40:08'),
(6, 1, 5, 'Legendary Mystery Box', 'legendary-box', 'Contains a chance to win 50,000 GASHY or a Rare NFT. High risk, high reward.', 500.000000000, 20.00, 995, 'mystery_box', '[\"https://cdn-icons-png.flaticon.com/512/1162/1162951.png\"]', NULL, 'active', 3001, '2026-01-16 08:40:08'),
...
(8, 1, 5, 'Starter Mystery Box', 'starter-box', 'Cheap box for testing luck.', 10.000000000, NULL, 498, 'mystery_box', '[\"https://cdn-icons-png.flaticon.com/512/4213/4213650.png\"]', NULL, 'active', 6, '2026-01-17 12:23:19');
...
INSERT INTO `sellers` (`user_id`, `store_name`, `store_slug`, `commission_rate`, `rating`, `total_sales`, `is_approved`) VALUES
...
INSERT INTO `transactions` (`id`, `user_id`, `type`, `amount`, `tx_signature`, `reference_id`, `status`, `created_at`) VALUES
...
(7, 1, 'purchase', 10.000000000, '108b1ed1167f1e1b0a842f137853c5b0f232f89573520932f688001bc0912b54d1460cf96f9de189454267b7', NULL, 'confirmed', '2026-01-18 02:08:14'),
(8, 1, 'reward', 5.000000000, NULL, 8, 'confirmed', '2026-01-18 02:08:14');
...
INSERT INTO `users` (`id`, `wallet_address`, `username`, `email`, `role`, `tier`, `nonce`, `is_verified`, `created_at`, `updated_at`, `is_banned`, `my_referral_code`) VALUES
...
INSERT INTO `user_quests` (`id`, `user_id`, `quest_id`, `progress`, `is_claimed`, `updated_at`) VALUES
(1, 1, 1, 10, 0, '2026-01-18 02:08:14');
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `view_products_marketplace`  AS SELECT `p`.`id` AS `id`, `p`.`title` AS `title`, `p`.`slug` AS `slug`, `p`.`price_gashy` AS `price_gashy`, `p`.`type` AS `type`, `p`.`images` AS `images`, `p`.`stock` AS `stock`, `c`.`name` AS `category_name`, `c`.`slug` AS `category_slug`, `s`.`store_name` AS `store_name`, `s`.`rating` AS `seller_rating`, `s`.`is_approved` AS `is_approved` FROM ((`products` `p` join `categories` `c` on(`p`.`category_id` = `c`.`id`)) join `sellers` `s` on(`p`.`seller_id` = `s`.`user_id`)) WHERE `p`.`status` = 'active' AND `p`.`stock` > 0 AND `s`.`is_approved` = 1 ;
...
ALTER TABLE `lottery_entries`
  ...
  ADD CONSTRAINT `fk_lot_user` FOREIGN KEY (`user_id`) REFERENCES `users` (`id`);
...
ALTER TABLE `products`
  ...
  ADD CONSTRAINT `fk_prod_seller` FOREIGN KEY (`seller_id`) REFERENCES `sellers` (`user_id`);
...
--
ALTER TABLE `user_referrals`
  ADD CONSTRAINT `user_referrals_ibfk_1` FOREIGN KEY (`referrer_id`) REFERENCES `users` (`id`),
  ADD CONSTRAINT `user_referrals_ibfk_2` FOREIGN KEY (`referee_id`) REFERENCES `users` (`id`);
COMMIT;
