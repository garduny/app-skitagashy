...
if (!$account_session) encode(['status' => false, 'message' => 'Unauthorized']);
$uid = $account_session['account_id'];
$page = max(1, (int)(request('page', 'get') ?? 1));
$limit = max(1, (int)(request('limit', 'get') ?? 10));
$offset = ($page - 1) * $limit;
$sql = " SELECT t.amount as my_bid_usd,t.created_at as bid_time,a.id as auction_id,a.status,a.end_time,a.current_bid_usd as highest_bid_usd,p.title,p.images,p.slug FROM transactions t JOIN auctions a ON t.reference_id=a.id JOIN products p ON a.product_id=p.id WHERE t.account_id=$uid AND t.type='auction_bid' ORDER BY t.created_at DESC LIMIT $limit OFFSET $offset ";
$data = getQuery($sql);
$rate = toGashy();
foreach ($data as &$r) {
    $r['my_bid'] = (float)$r['my_bid_usd'] / $rate;
    $r['highest_bid'] = (float)$r['highest_bid_usd'] / $rate;
    unset($r['my_bid_usd'], $r['highest_bid_usd']);
}
