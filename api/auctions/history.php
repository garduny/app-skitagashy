...
$offset = ($page - 1) * $limit;
$sql = " SELECT t.amount as my_bid,t.created_at as bid_time,a.id as auction_id,a.status,a.end_time,a.current_bid as highest_bid,p.title,p.images,p.slug FROM transactions t JOIN auctions a ON t.reference_id=a.id JOIN products p ON a.product_id=p.id WHERE t.user_id=$uid AND t.type='auction_bid' ORDER BY t.created_at DESC LIMIT $limit OFFSET $offset ";
$data = getQuery($sql);
$total = countQuery(" SELECT COUNT(*) FROM transactions WHERE user_id=$uid AND type='auction_bid' ");
