...
if ($filter === 'hot') $order = " ORDER BY a.current_bid_usd DESC ";
...
$sql = " SELECT a.id,a.end_time,a.current_bid_usd,a.start_price_usd,a.status,p.title,p.images,p.slug,p.description,u.accountname as high_bidder FROM auctions a JOIN products p ON a.product_id=p.id LEFT JOIN accounts u ON a.highest_bidder_id=u.id $where $order LIMIT $limit OFFSET $offset ";
$data = getQuery($sql);
$total = countQuery(" SELECT 1 FROM auctions a $where ");
$rate = toGashy();
foreach ($data as &$row) {
    $row['current_bid'] = $row['current_bid_usd'] / $rate;
    $row['start_price'] = $row['start_price_usd'] / $rate;
    $row['time_left'] = strtotime($row['end_time']) - time();
    unset($row['current_bid_usd'], $row['start_price_usd']);
}
