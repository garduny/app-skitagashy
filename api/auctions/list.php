...
foreach ($data as &$row) {
    ...
    $baseBid = max((float)$row['current_bid_usd'], (float)$row['start_price_usd']);
    $nextUsd = $baseBid + max(1, $baseBid * 0.05);

    $row['next_min_bid'] = round($nextUsd / $rate, 8);

    unset(
        $row['images'],
        $row['current_bid_usd'],
        $row['start_price_usd'],
        $row['reserve_price_usd']
    );
}
