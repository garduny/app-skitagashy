...
$products = getQuery(" SELECT p.id,p.title,p.description,p.price_usd,p.stock,p.type,p.status,p.images,c.name cat_name FROM products p LEFT JOIN categories c ON p.category_id=c.id WHERE p.seller_id=$uid ORDER BY p.id DESC ");
foreach ($products as &$pr) {
    $pr['price_gashy'] = $rate > 0 ? $pr['price_usd'] / $rate : 0;
}
...
$total_units = findQuery(" SELECT COALESCE(SUM(oi.quantity),0) q FROM order_items oi JOIN products p ON oi.product_id=p.id JOIN orders o ON oi.order_id=o.id WHERE p.seller_id=$uid AND o.status='completed' ")['q'] ?? 0;
$stats = ['total_units' => (int)$total_units, 'total_sales' => number_format($gross, 3, '.', ''), 'rating' => $seller['rating'], 'products' => $product_count, 'earnings' => number_format($net_earnings, 3, '.', ''), 'reward_balance' => number_format($reward_balance, 3, '.', ''), 'failed_bid' => number_format($failedbid, 3, '.', ''), 'available' => number_format($available, 3, '.', ''), 'gross_sales' => number_format($gross, 3, '.', ''), 'fee_rate' => $fee_percent . '%'];
encode(['status' => true, 'seller' => $seller, 'stats' => $stats, 'products' => $products, 'sales' => $sales, 'withdrawals' => $withdrawals]);
