...
$box = findQuery(" SELECT id,title,price_usd,stock FROM products WHERE id=$box_id AND seller_id=$uid AND type='mystery_box' AND status!='deleted' ");
if (!$box) {
    ob_clean();
...
if ($action === 'list') {
    $loot = getQuery(" SELECT l.*,p.title FROM mystery_box_loot l LEFT JOIN products p ON l.reward_product_id=p.id WHERE l.box_product_id=$box_id ORDER BY l.probability ASC ");
    ...
    foreach ($products as &$p) {
        $p['price_gashy'] = $rate > 0 ? $p['price_usd'] / $rate : 0;
    }
    ob_clean();
    encode(['status' => true, 'loot' => $loot, 'products' => $products, 'box' => ['id' => $box['id'], 'title' => $box['title'], 'price' => $rate > 0 ? $box['price_usd'] / $rate : 0, 'stock' => $box['stock']]]);
}
if ($action === 'add') {
    $reward_product_id = !empty(request('reward_product_id')) ? (int)request('reward_product_id') : null;
    $reward_option_id = !empty(request('reward_option_id')) ? (int)request('reward_option_id') : 0;
    ...
    $current_total = findQuery(" SELECT COALESCE(SUM(probability),0) t FROM mystery_box_loot WHERE box_product_id=$box_id ")['t'] ?? 0;
    ...
    execute(" INSERT INTO mystery_box_loot (box_product_id,reward_product_id,reward_option_id,reward_amount,probability,rarity) VALUES ($box_id,$rpVal,$reward_option_id,$reward_amount,$probability,'$rarity') ");
    ob_clean();
