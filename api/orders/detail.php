...
$oid = (int)request('id');
if (!$oid) encode(['status' => false, 'message' => 'Order ID required']);
...
if ($order['status'] !== 'completed') encode(['status' => false, 'message' => 'Order not paid yet. Content is locked.']);
$cards = getQuery(" SELECT p.title,gc.id,gc.code_enc,gc.pin_enc FROM gift_cards gc JOIN products p ON gc.product_id=p.id WHERE gc.sold_to_order_id=$oid ORDER BY gc.id ASC ");
...
encode(['status' => true, 'order_id' => $oid, 'gift_cards' => $delivered_items]);
