...
if ($tab === 'sold' && $is_seller) {
    $sql = " SELECT DISTINCT o.id,o.total_gashy,o.status,o.created_at,o.tx_signature,a.accountname AS buyer FROM orders o JOIN order_items oi ON oi.order_id=o.id JOIN products p ON oi.product_id=p.id JOIN accounts a ON a.id=o.account_id WHERE p.seller_id=$uid ORDER BY o.id DESC LIMIT $limit OFFSET $offset ";
    $total_sql = " SELECT COUNT(DISTINCT o.id) FROM orders o JOIN order_items oi ON oi.order_id=o.id JOIN products p ON oi.product_id=p.id WHERE p.seller_id=$uid ";
    $can_edit = true;
} else {
...
encode(['status' => true, 'data' => $orders, 'meta' => ['page' => $page, 'total' => $total], 'is_seller' => $is_seller, 'can_edit' => $can_edit]);
