...
$burnTx = trim((string)($data['burn_tx'] ?? ''));
$tickets = (int)($data['tickets'] ?? 0);
$postedRoundId = (int)($data['round_id'] ?? 0);
...
try {
    ...
    $round = findQuery(" SELECT id,draw_time,status FROM lottery_rounds WHERE status='open' ORDER BY id DESC LIMIT 1 FOR UPDATE ");
    if (!$round) {
        execute(" ROLLBACK ");
