...
$openCount = countQuery(" SELECT 1 FROM lottery_rounds WHERE status='open' ");
if ($openCount > 1) {
    $keep = findQuery(" SELECT id FROM lottery_rounds WHERE status='open' ORDER BY id DESC LIMIT 1 ");
    if ($keep) {
        $keepId = (int)$keep['id'];
        execute(" UPDATE lottery_rounds SET status='closed' WHERE status='open' AND id<>$keepId ");
    }
}
$round = findQuery(" SELECT * FROM lottery_rounds WHERE status='open' ORDER BY id DESC LIMIT 1 ");
if (!$round) {
    $lastRound = findQuery(" SELECT MAX(round_number) as round_number FROM lottery_rounds ");
    $nextRound = ((int)($lastRound['round_number'] ?? 0)) + 1;
    $nextDraw = date('Y-m-d H:i:s', strtotime('+7 days'));
    execute(" INSERT INTO lottery_rounds (round_number,prize_pool,draw_time,status) VALUES ($nextRound,0,'$nextDraw','open') ");
    $round = findQuery(" SELECT * FROM lottery_rounds WHERE status='open' ORDER BY id DESC LIMIT 1 ");
}
$rid = (int)$round['id'];
...
$lastRound = findQuery(" SELECT * FROM lottery_rounds WHERE status='closed' AND winning_numbers IS NOT NULL ORDER BY id DESC LIMIT 1 ");
$lastWinners = [];
if ($lastRound) {
...
WHERE le.round_id=$lrid AND (le.is_winner='yes' OR le.is_winner=1)
ORDER BY le.ticket_count DESC,le.id DESC
");
