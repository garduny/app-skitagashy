...
try {
    execute(" INSERT INTO transactions (user_id,type,amount,tx_signature,status,created_at) VALUES ($uid,'purchase',{$box['price_gashy']},'$txSig','confirmed',NOW()) ");
    ...
    if ($wonItem['reward_product_id']) {
        ...
        if ($prod && $prod['stock'] > 0) {
            execute(" INSERT INTO orders (user_id,total_gashy,tx_signature,status,created_at) VALUES ($uid,0,'$txSig-REWARD','completed',NOW()) ");
            ...
            execute(" INSERT INTO transactions (user_id,type,amount,reference_id,status,created_at) VALUES ($uid,'reward',$fallbackAmount,$boxId,'confirmed',NOW()) ");
            $rewardData = ['type' => 'token', 'amount' => $fallbackAmount, 'note' => 'Fallback (OOS)'];
        }
    } else {
        $amount = $wonItem['reward_amount'];
        execute(" INSERT INTO transactions (user_id,type,amount,reference_id,status,created_at) VALUES ($uid,'reward',$amount,$boxId,'confirmed',NOW()) ");
