...
try {
    ...
    if (!$wonItem) $wonItem = $loot[count($loot) - 1];
    $rewardData = [];
    if ($wonItem['reward_product_id']) {
        $pid = (int)$wonItem['reward_product_id'];
        $prod = findQuery(" SELECT price_usd,type,stock FROM products WHERE id=$pid ");
        if ($prod && $prod['stock'] > 0) {
            ...
            if ($prod['type'] === 'gift_card' || $prod['type'] === 'digital') {
                ...
                if ($card) execute(" UPDATE gift_cards SET is_sold=1,sold_to_order_id=$oid WHERE id={$card['id']} ");
            }
            $rewardData = ['type' => 'product', 'product_id' => $pid];
        } else {
            $fallbackAmount = $boxPriceGashy / 2;
        ...
        $amount = (float)$wonItem['reward_amount'];
