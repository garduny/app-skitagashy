...
$offset = ($page - 1) * $limit;
$rate = toGashy();
$sql = " SELECT p.id,p.title,p.slug,p.price_usd,p.images,p.description,p.stock FROM products p WHERE p.type='mystery_box' AND p.status='active' AND p.stock>0 ORDER BY p.price_usd ASC LIMIT $limit OFFSET $offset ";
$data = getQuery($sql);
