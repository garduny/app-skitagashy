...
$sql = " SELECT p.*,c.name as category_name,c.slug as category_slug,s.store_name,s.rating as seller_rating,s.user_id as seller_id FROM products p JOIN categories c ON p.category_id=c.id JOIN sellers s ON p.seller_id=s.user_id WHERE $cond AND p.status='active' LIMIT 1 ";
