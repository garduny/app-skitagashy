...
$sql = " SELECT p.id,p.title,p.slug,p.price_gashy,p.images,p.type,p.stock,c.name as category_name,c.slug as category_slug,s.store_name,s.rating as seller_rating FROM products p JOIN categories c ON p.category_id=c.id JOIN sellers s ON p.seller_id=s.user_id $where $order LIMIT $limit OFFSET $offset ";
$data = getQuery($sql);
