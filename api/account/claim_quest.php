...
if (!$qid) encode(['status' => false, 'message' => 'Invalid Quest']);
execute(" START TRANSACTION ");
try {
    $q = findQuery(" SELECT q.id,q.target_count,q.reward_gashy,IFNULL(aq.progress,0) progress,IFNULL(aq.is_claimed,0) is_claimed FROM quests q LEFT JOIN account_quests aq ON q.id=aq.quest_id AND aq.account_id=$uid WHERE q.id=$qid ");
    if (!$q) throw new Exception("Quest not found");
    if ($q['progress'] < $q['target_count']) throw new Exception("Quest not finished yet");
    if ($q['is_claimed']) throw new Exception("Reward already claimed");
